...
class Knight(IRobot):
	...
	pathfindingController, missionController, unit, map_controller):
	...
	def try_attack(self, target_robot_id):
		...
		if not self.game_controller.is_attack_ready(self.unit.id):
			print("Knight [{}] attack is not ready. Not enough heat".format(self.unit.id))
			return False
	...
	def try_javelin(self, target_robot_id):
		"""Trys the javelin"""
		# Checks that the knight has fully been researched so that it can now throw Javelins at enemy units
		level = bc.researh_info.get_level(bc.UnitType.Knight)
		if level != 3:
			return False
